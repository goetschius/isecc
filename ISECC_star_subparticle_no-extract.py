...
def formatVertexAssignment( particle, sym_index, ROI ):
    ...
    fivefold_general   = str(int(scalar_at(particle['Vertex_5f_general'], sym_index))).rjust(2, '0')
    fivefold_specific  = decode_if_bytes(scalar_at(particle['Vertex_5f_specific'], sym_index))

    threefold_general  = str(int(scalar_at(particle['Vertex_3f_general'], sym_index))).rjust(2, '0')
    threefold_specific = decode_if_bytes(scalar_at(particle['Vertex_3f_specific'], sym_index))

    twofold_general    = str(int(scalar_at(particle['Vertex_2f_general'], sym_index))).rjust(2, '0')
    twofold_specific   = decode_if_bytes(scalar_at(particle['Vertex_2f_specific'], sym_index))
...
#    user_vector = np.array( [0, 0.618, 1] )
    ...
    for particle_index, particle in enumerate(star_array):
        ...
        for index, item in enumerate(this_particle):
            ...
            this_particle[index]['ExpandQuatIndex'] = np.asarray(master_table[particle_index]['ExpandQuatIndex']).item(index)
...
def defineSubparticles( my_ndarray, ROI, user_vector, user_fudge, user_subbox, higher_order_sym, user_testmode, user_batch_size, header, fullheader, RUN_ID, regen_string, user_batch=None ) :
    ...
    print( 'User supplied vector ', user_vector )
