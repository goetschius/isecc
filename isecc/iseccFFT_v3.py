...
#    my_ndimage = grid_correct(my_ndimage, pfac=2, order=1)
...
#    mag_shifted = np.real(myFFT_shifted)
#    phase_shifted = np.imag(myFFT_shifted)

    mag_shifted = np.abs(myFFT_shifted)
    phase_shifted = np.angle(myFFT_shifted)
    ...
    freq = np.fft.fftfreq(box_size, d=sample_rate)
    freq_shift = np.fft.fftshift(freq)
#    freq_shift = np.reciprocal( freq_shift )
   ####
    ...
    f_mag = scipy.interpolate.RegularGridInterpolator((x,y,z),mag_shifted,bounds_error=False, fill_value=0)
    f_phase = scipy.interpolate.RegularGridInterpolator((x,y,z),phase_shifted,bounds_error=False, fill_value=0)
...
#    rotated_mag = f_mag(coordStack_rotated)
#    rotated_phase = f_phase(coordStack_rotated)
