...
#					if np.allclose(reference_xyz_icos, negate_compare_xyz_icos, atol=2) and not np.array_equal(reference_xyz_icos, compare_xyz_icos) and (compare_index not in assessed_diameter):
#					if np.allclose(reference_xyz_icos, negate_compare_xyz_icos, atol=2) and not (reference_xyz_icos, compare_xyz_icos):
					if np.allclose(reference_xyz_icos, negate_compare_xyz_icos, atol=1):

#						print( '\nDEBUG2:', reference_xyz_icos, negate_compare_xyz_icos, '\n' )

						### Add to list to prevent duplicate assessment
...
#						print( reference_xyz_icos, compare_xyz_icos )
#						print( ideal_polar_distance, flattened_ideal_polar_distance, flattened_real_polar_distance )
#						print( '\nParticle', particle_specifier, 'expected diameter is:', flattened_ideal_polar_distance )

#						print( 'Expected diameter:', flattened_ideal_polar_distance )
#						print( 'Real diameter is', flattened_real_polar_distance, 'a difference ratio of:', np.around( np.true_divide(flattened_real_polar_distance,flattened_ideal_polar_distance), decimals=4) )
						print( 'Difference ratio: ', np.around( np.true_divide(flattened_real_polar_distance,flattened_ideal_polar_distance), decimals=4) )

#					else:
#						print( 'DEBUG3:', reference_xyz_icos, compare_xyz_icos )
...
#		sys.exit()
		...
		### Make sub array containing the pent and hex capsomers on this vertex
#		condition = subparticles['Vertex5f_general'] == unique_vertex
#		this_vertex = subparticles[ condition ]
#		print( '\nThis vertex:\n', this_vertex )


		for pentavalent in pentavalents:

#			print( pentavalent['SubparticleRelative_XYZ'][2] )

			if (pentavalent['SubparticleRelative_XYZ'][2] > z_threshold) or (pentavalent['SubparticleRelative_XYZ'][2] < (-1*z_threshold)):

#				print( '\nThis pentavalent on this vertex, on this particle:\n', pentavalent['SubparticleRelative_XYZ'] )
#				print( '\nThis pentavalent on this vertex, on this particle:\n', pentavalent['SubparticleOrigin_icos'] )
	
				condition = hexavalents['Vertex5f_general'] == pentavalent['Vertex5f_general']
				hexavalents_this_vertex = hexavalents[ condition ]

				### Sort hexavalents_this_vertex from a --> e
				hexavalents_this_vertex.sort(order='Vertex5f_specific')
					
#				print( '\nHexavalents in this vertex, on this particle:\n', hexavalents_this_vertex['SubparticleRelative_XYZ'] )
#				print( '\nHexavalents in this vertex, on this particle:\n', hexavalents_this_vertex['SubparticleOrigin_icos'], '\n' )


				### Relevant parameters
				pent_x_icos  = pentavalent['SubparticleRelative_XYZ'][0]
				pent_y_icos  = pentavalent['SubparticleRelative_XYZ'][1]
				pent_z_icos  = pentavalent['SubparticleRelative_XYZ'][2]

				### Value >0 if localOrigin > icosOrigin. Verify the sign!
				pent_delta_x = pentavalent['SubparticleOrigin_local'][0] - pentavalent['SubparticleOrigin_icos'][0]
				pent_delta_y = pentavalent['SubparticleOrigin_local'][1] - pentavalent['SubparticleOrigin_icos'][1]

				### Add in the deltas
				pent_x_local = pentavalent['SubparticleRelative_XYZ'][0] + pent_delta_x
				pent_y_local = pentavalent['SubparticleRelative_XYZ'][1] + pent_delta_y
	
				### Flatten along z
				pent_xy_icos = np.array( [ pent_x_icos, pent_y_icos, 0 ] )
				pent_xy_local = np.array( [ pent_x_local, pent_y_local, 0 ] )
	
				### Maintain z
				pent_xyz_icos = np.array( [ pent_x_icos, pent_y_icos, pent_z_icos ] )

				ideal_distance_list=[]
				real_distance_list=[]
				delta_list=[]	
	
				for hexavalent in hexavalents_this_vertex:

					hex_x_icos  = hexavalent['SubparticleRelative_XYZ'][0]
					hex_y_icos  = hexavalent['SubparticleRelative_XYZ'][1]
					hex_z_icos  = hexavalent['SubparticleRelative_XYZ'][2]

					## Value >0 if localOrigin > icosOrigin. Verify the sign!
					hex_delta_x = hexavalent['SubparticleOrigin_local'][0] - hexavalent['SubparticleOrigin_icos'][0]
					hex_delta_y = hexavalent['SubparticleOrigin_local'][1] - hexavalent['SubparticleOrigin_icos'][1]

					### Add in the deltas
					hex_x_local = hexavalent['SubparticleRelative_XYZ'][0] + hex_delta_x
					hex_y_local = hexavalent['SubparticleRelative_XYZ'][1] + hex_delta_y

					### Flatten along z
					hex_xy_icos = np.array( [ hex_x_icos, hex_y_icos, 0 ] )
					hex_xy_local = np.array( [ hex_x_local, hex_y_local, 0 ] )
	
					### Maintain z
					hex_xyz_icos = np.array( [ hex_x_icos, hex_y_icos, hex_z_icos ] )
	
					### Run distance calculations
					ideal_distance = np.around( assess3dDistance( pent_xyz_icos,  hex_xyz_icos ), decimals=1 )
					ideal_distance_flattened = np.around( assess3dDistance( pent_xy_icos,  hex_xy_icos ), decimals=1 )
					real_distance_flattened  = np.around( assess3dDistance( pent_xy_local, hex_xy_local ), decimals=1 )
					delta_flattened = np.around( assess3dDistance( pent_xy_icos,  hex_xy_icos ) - assess3dDistance( pent_xy_local, hex_xy_local ), decimals=1 )

#					print( 'Hex to pent distance (ideal) is:', ideal_distance )
	
#					print( 'Ideal distance is:', ideal_distance_flattened )
#					print( 'Real distance is : ', real_distance_flattened  )
#					print( 'Delta is:         ', delta_flattened, '\n' )
				...
				vertex_specifier = str( int( pentavalent['Vertex5f_general'] ) ).rjust(2, '0')
				vertex_specifier = ''.join( [ '5f', vertex_specifier ] )
				print( 'Deltas for vertex', vertex_specifier, 'on particle', particle_specifier, 'are:', delta_list )
			...
			# Grab the pentavalent capsomer
#			condition = (subparticles['Vertex5f_general'] == unique_vertex) and (subparticles['Subparticle_type'] == 'pentavalent')


#			this_pentavalent = subparticles[ ('Vertex5f_general' == unique_vertex ) and ( 'Subparticle_type' == 'pentavalent' ) ]


#			this_pentavalent = np.extract( [condition.all(), subparticles )

#			print( 'this pentavalent', this_pentavalent )
