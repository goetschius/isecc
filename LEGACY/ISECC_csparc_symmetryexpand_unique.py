...
def load_symops_module():
    ...
    if not module_path.exists():
        module_path = Path(__file__).resolve().parent.parent / "isecc" / "symops.py"
...
def no_expand_output_path_for(input_path, extra_suffix=""):
    input_path = Path(input_path)
    return input_path.with_name(f"{input_path.stem}_noSymExpand{extra_suffix}.cs")
...
def apply_z_offset_without_expansion(
    ...
    if subparticle_distance_angstrom is None:
        raise ValueError("--subparticle-distance is required with --no-symmetry-expand.")
    ...
    if output_path is None:
        output_path = no_expand_output_path_for(input_path, extra_suffix="_zShift")
...
def apply_z_offset_and_combine_without_expansion(
    ...
    if subparticle_distance_angstrom is None:
        raise ValueError("--subparticle-distance is required with --no-symmetry-expand.")
    if output_path is None:
        raise ValueError("--output is required when combining no-expansion Z-shift inputs.")
...
def symmetry_expand_unique_particles(
    ...
    if legacy_i2_i1_frame and symmetry != "I2":
        raise ValueError("--legacy-i2-i1-frame is only valid with --symmetry I2.")

    operator_symmetry = "I1" if legacy_i2_i1_frame else symmetry
    legacy_frame_quat = Quaternion(axis=(0.0, 0.0, 1.0), degrees=90) if legacy_i2_i1_frame else Quaternion()

    quaternions = symops.getSymOps(operator_symmetry)
    if legacy_i2_i1_frame:
        unique_indices = inverse_unique_vertex_symop_indices(operator_symmetry, vertex)
    else:
        unique_indices = unique_vertex_symop_indices(operator_symmetry, vertex)
    ideal_vector = IDEAL_VECTORS[operator_symmetry][vertex]
    target_z = np.array((0.0, 0.0, 1.0), dtype=np.float32)
    csparc_quat_to_z = csparc_roi_quat_to_z(operator_symmetry, vertex)
    assert_no_duplicate_subparticles_for_sampled_particles(
        ...
        vertex,
        symmetry,
        legacy_frame_quat=legacy_frame_quat,
    )
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "For I2 datasets that should match historical ISECC behavior, also use --legacy-i2-i1-frame."
        ),
    )
    parser.add_argument(
        "--no-align-to-z",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "but does not reproduce the historical ISECC I1 fivefold convention. "
            "Inverse and custom-vector modes are convention checks."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "for --symmetry I2, reproduce the historical ISECC convention: "
            "rotate input poses +90 degrees about Z into the I1 frame, then use I1 ROI operators"
        ),
    )
    parser.add_argument(
        "--no-symmetry-expand",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "with --no-symmetry-expand, repeat this for multiple input .cs files to "
    ...
    if args.no_symmetry_expand:
        if align_to_z:
            parser.error("--align-to-z is not used with --no-symmetry-expand because poses are preserved.")
    ...
    if args.vertex is None:
        if args.custom_vector is None:
            parser.error("--vertex/--roi is required unless --no-symmetry-expand or --custom-vector is set.")
    if args.input is None:
        parser.error("INPUT is required unless --no-symmetry-expand with --combine-input is used.")
    ...
    output_path, input_count, output_count, unique_indices = symmetry_expand_unique_particles(
        ...
        legacy_i2_i1_frame=args.legacy_i2_i1_frame,
    )
    print(f"Input particles:  {input_count}")
    ...
    if args.subparticle_distance is not None:
        print(f"Subparticle distance: {args.subparticle_distance:.3f} A")
    if args.legacy_i2_i1_frame:
        print("Legacy I2->I1 frame: True")
        if args.write_debug_shift_plus:
